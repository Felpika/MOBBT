...
def gerar_grafico_ettj_longo_prazo(df):
    ...
    return fig
# ====== AUX: maturidade (anos) e cross-section por dia ======
def _maturity_in_years(base: pd.Series, venc: pd.Series) -> pd.Series:
    return (venc - base).dt.days / 365.25

def _ntnb_cross_section(df_tesouro: pd.DataFrame, data_base: pd.Timestamp) -> pd.DataFrame:
    tipos_ntnb = ['Tesouro IPCA+', 'Tesouro IPCA+ com Juros Semestrais']
    df = df_tesouro[(df_tesouro['Tipo Titulo'].isin(tipos_ntnb)) & (df_tesouro['Data Base'] == data_base)].copy()
    if df.empty:
        return df
    df['T'] = _maturity_in_years(df['Data Base'], df['Data Vencimento'])
    df = df[df['T'] > 0].sort_values('T')
    return df[['Data Vencimento','Tipo Titulo','T','Taxa Compra Manha','PU Compra Manha']]

# ====== 1A) Duração/Convexidade empíricas (rolling regressão) ======
@st.cache_data
def estimar_dur_conv_empirico(df_tesouro: pd.DataFrame, janela_dias: int = 60) -> pd.DataFrame:
    """
    Para cada vencimento de NTN-B, estima 'Duration' e 'Convexity' empíricos
    a partir da relação histórica Δln(PU) ~ -D*Δy + 0.5*C*(Δy)^2 (janela móvel).
    Saída: último estimate por vencimento na data mais recente disponível.
    """
    tipos_ntnb = ['Tesouro IPCA+', 'Tesouro IPCA+ com Juros Semestrais']
    df = df_tesouro[df_tesouro['Tipo Titulo'].isin(tipos_ntnb)].copy()
    if df.empty: 
        return pd.DataFrame()

    out = []
    for venc, grp in df.groupby('Data Vencimento'):
        g = grp.sort_values('Data Base').dropna(subset=['Taxa Compra Manha','PU Compra Manha'])
        if g.shape[0] < janela_dias + 5:
            continue
        # difs diárias
        dy = g['Taxa Compra Manha'].pct_change(1, fill_method=None)  # var % pequena -> aproxima Δy/y; robusto a níveis altos/baixos
        # melhor usar var em pontos (decimal); convertemos taxa %a.a. -> decimal
        dy_bps = (g['Taxa Compra Manha']/100.0).diff()  # Δy em termos absolutos (decimal)
        dlnP = np.log(g['PU Compra Manha']).diff()

        X1 = dy_bps.rolling(janela_dias).apply(lambda s: s.iloc[-1] if s.notna().all() else np.nan)  # placeholder para alinhar
        # Faz regressão polinomial 2º grau sobre a janela final
        # Seleciona últimos 'janela_dias' pontos válidos
        mask = (~dy_bps.isna()) & (~dlnP.isna())
        dyw = dy_bps[mask].tail(janela_dias)
        dPw = dlnP[mask].tail(janela_dias)
        if dyw.size < 20:
            continue
        # Ajuste: dlnP = a + b*dy + c*dy^2  -> D ≈ -b ; C ≈ 2*c
        coeffs = np.polyfit(dyw.values, dPw.values, deg=2)
        a, b, c = coeffs[2], coeffs[1], coeffs[0]
        D = max(0.0, -b)         # duration empírica (não-negativa)
        C = max(0.0, 2.0*c)      # convexidade empírica aproximada
...
# ====== 1B) Carry & roll-down (proxy) ======
def _interp_yield_at(df_cs: pd.DataFrame, target_T: float) -> float:
    # interpola linearmente na cross-section do dia
    x = df_cs['T'].values
    y = (df_cs['Taxa Compra Manha']/100.0).values  # decimal
    if target_T <= x.min(): return float(y[0])
    if target_T >= x.max(): return float(y[-1])
    return float(np.interp(target_T, x, y))

@st.cache_data
def calcular_carry_rolldown_ntnb(df_tesouro: pd.DataFrame, horizonte_dias: int = 30) -> pd.DataFrame:
    """
    Proxy de retorno 1M/3M/6M usando:
    retorno ≈ -D*Δy_rd + 0.5*C*(Δy_rd)^2  (+ carry de cupom simples p/ NTNB semestral, proporcional ao horizonte)
    Onde Δy_rd é a variação de yield por 'rolar' T -> T - h/365 no mesmo snapshot de curva.
    """
    if df_tesouro.empty: return pd.DataFrame()
    ultima_data = df_tesouro['Data Base'].max()
    cs = _ntnb_cross_section(df_tesouro, ultima_data)
    if cs.empty: return pd.DataFrame()

    durconv = estimar_dur_conv_empirico(df_tesouro)
    if durconv.empty: return pd.DataFrame()

    h_years = horizonte_dias/365.25
    rows = []
    for _, r in cs.iterrows():
        T_now = r['T']
        y_now = r['Taxa Compra Manha']/100.0
        T_fwd = max(0.05, T_now - h_years)
        y_fwd = _interp_yield_at(cs, T_fwd)
        dy_rd = (y_fwd - y_now)

        drow = durconv[durconv['Data Vencimento']==r['Data Vencimento']]
        if drow.empty: 
            continue
        D = float(drow['Duration_emp'].iloc[0])
        C = float(drow['Convexity_emp'].iloc[0])

        ret_price = (-D * dy_rd) + (0.5 * C * (dy_rd**2))   # retorno aproximado (preço)
        carry_cupom = 0.0
        if r['Tipo Titulo'] == 'Tesouro IPCA+ com Juros Semestrais':
            # cupom real ~ taxa/2 ao semestre; rateamos por horizonte em meses/6
            carry_cupom = (y_now/2.0) * (horizonte_dias/182.6)

        rows.append({
            'Vencimento': r['Data Vencimento'],
            'Tipo': r['Tipo Titulo'],
            'T (anos)': round(T_now,2),
            'Taxa atual (%aa)': r['Taxa Compra Manha'],
            f'Ret.Rolldown ~{horizonte_dias}d (%)': 100.0*ret_price,
            f'Carry cupom ~{horizonte_dias}d (%)': 100.0*carry_cupom,
            f'Ret.Total ~{horizonte_dias}d (%)': 100.0*(ret_price + carry_cupom),
            'Duration_emp': D,
            'Convexity_emp': C
        })
    df_out = pd.DataFrame(rows).sort_values('Vencimento')
    return df_out

# ====== 1C) Breakeven de inflação – curva (snapshot do dia) ======
def calcular_curva_breakeven(df_tesouro: pd.DataFrame) -> pd.DataFrame:
    """
    Para a data mais recente, pareia prefixados vs IPCA por maturidade próxima
    e interpola a BE ao longo de T (anos). Saída: DataFrame com T e BE (%aa).
    """
    if df_tesouro.empty: return pd.DataFrame()
    d = df_tesouro['Data Base'].max()
    df_dia = df_tesouro[df_tesouro['Data Base']==d].copy()
    if df_dia.empty: return pd.DataFrame()

    pref = df_dia[df_dia['Tipo Titulo']=='Tesouro Prefixado'].copy()
    ipca = df_dia[df_dia['Tipo Titulo'].isin(['Tesouro IPCA+','Tesouro IPCA+ com Juros Semestrais'])].copy()
    if pref.empty or ipca.empty: return pd.DataFrame()

    pref['T'] = _maturity_in_years(pref['Data Base'], pref['Data Vencimento'])
    ipca['T'] = _maturity_in_years(ipca['Data Base'], ipca['Data Vencimento'])
    pref = pref[(pref['T']>0)].sort_values('T')
    ipca = ipca[(ipca['T']>0)].sort_values('T')

    Ts = np.linspace(max(0.25, min(pref['T'].min(), ipca['T'].min())),
                     min(pref['T'].max(), ipca['T'].max()), 60)
    yp = np.interp(Ts, pref['T'], pref['Taxa Compra Manha']/100.0)
    yi = np.interp(Ts, ipca['T'], ipca['Taxa Compra Manha']/100.0)
    be = ((1+yp)/(1+yi) - 1.0) * 100.0  # em % a.a.
    return pd.DataFrame({'T (anos)': Ts, 'Breakeven (%aa)': be})

def grafico_curva_breakeven(df_be: pd.DataFrame):
    if df_be.empty: 
        return go.Figure().update_layout(title_text="Breakeven: dados insuficientes", template='plotly_dark')
    fig = px.line(df_be, x='T (anos)', y='Breakeven (%aa)', template='plotly_dark',
                  title='Curva de Breakeven de Inflação (snapshot do dia)')
    fig.update_layout(title_x=0, xaxis_title='Maturidade (anos)', yaxis_title='BE (% a.a.)')
    return fig
# ====== 2) Heatmap de inclinações (LTN/NTN-F) ======
@st.cache_data
def calcular_inclinacoes_prefixados(df_tesouro: pd.DataFrame,
                                    alvos_anos = (1,2,5,10),
                                    janelas=['D-1','D-5','YTD']) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Constrói uma série diária de taxas nos tenores-alvo via interpolação
    (usando LTNs/NTN-Fs no dia) e calcula inclinações (spreads entre tenores).
    Retorna: (df_taxas_tenores, df_spreads_diarios)
    """
    dfp = df_tesouro[df_tesouro['Tipo Titulo'].isin(['Tesouro Prefixado','Tesouro Prefixado com Juros Semestrais'])].copy()
    if dfp.empty: return pd.DataFrame(), pd.DataFrame()

    taxas = []
    for data_base, dia in dfp.groupby('Data Base'):
        dia = dia.copy()
        dia['T'] = _maturity_in_years(dia['Data Base'], dia['Data Vencimento'])
        dia = dia[(dia['T']>0)].sort_values('T')
        if dia.empty: 
            continue
        x = dia['T'].values
        y = (dia['Taxa Compra Manha']).values
        row = {'Data Base': data_base}
        for t in alvos_anos:
            if t <= x.min(): row[f'{t}y'] = float(y[0])
            elif t >= x.max(): row[f'{t}y'] = float(y[-1])
            else: row[f'{t}y'] = float(np.interp(t, x, y))
        taxas.append(row)

    df_tenores = pd.DataFrame(taxas).set_index('Data Base').sort_index()
    if df_tenores.empty: return pd.DataFrame(), pd.DataFrame()
    # spreads clássicos
    cols = [f'{t}y' for t in alvos_anos]
    # exemplo: (2y-1y), (5y-2y), (10y-5y)
    pairs = [(2,1),(5,2),(10,5)]
    df_spreads = pd.DataFrame(index=df_tenores.index)
    for a,b in pairs:
        df_spreads[f'{a}y-{b}y'] = df_tenores[f'{a}y'] - df_tenores[f'{b}y']
    return df_tenores, df_spreads

def grafico_heatmap_inclinacoes(df_spreads: pd.DataFrame, ult_n=90):
    if df_spreads.empty:
        return go.Figure().update_layout(title_text="Inclinações: dados insuficientes", template='plotly_dark')
    data = df_spreads.tail(ult_n)
    fig = px.imshow(
        data.T, aspect='auto', origin='lower', template='plotly_dark',
        color_continuous_scale='RdBu_r',
        title='Mapa de Calor de Inclinações (Δtaxa, em p.p.)'
    )
    fig.update_layout(title_x=0, xaxis_title='Data', yaxis_title='Spread')
    return fig
...
if pagina_selecionada == "NTN-Bs":
    ...
    if not df_tesouro.empty:
        ...
        # === Após o fig_hist_ntnb ===
st.markdown("---")
st.subheader("Sensibilidade & Carry/Roll-down (aproximação)")

colh = st.columns(3)
with colh[0]:
    horizonte = st.selectbox("Horizonte", ["30 dias","90 dias","180 dias"], index=0)
    hmap = {"30 dias":30,"90 dias":90,"180 dias":180}
    H = hmap[horizonte]

# calcula uma vez
df_durconv = estimar_dur_conv_empirico(df_tesouro)
df_carry = calcular_carry_rolldown_ntnb(df_tesouro, horizonte_dias=H)

if not df_carry.empty:
    # destaque top por Ret.Total
    top = df_carry.sort_values(f'Ret.Total ~{H}d (%)', ascending=False).head(8)
    st.dataframe(
        top[['Vencimento','Tipo','T (anos)','Taxa atual (%aa)',
             f'Ret.Rolldown ~{H}d (%)', f'Carry cupom ~{H}d (%)', f'Ret.Total ~{H}d (%)',
             'Duration_emp','Convexity_emp'
        ]].style.format({
            'T (anos)': '{:.2f}',
            'Taxa atual (%aa)':'{:.2f}',
            f'Ret.Rolldown ~{H}d (%)':'{:+.2f}',
            f'Carry cupom ~{H}d (%)':'{:+.2f}',
            f'Ret.Total ~{H}d (%)':'{:+.2f}',
            'Duration_emp':'{:.2f}',
            'Convexity_emp':'{:.2f}'
        }),
    ...
    st.warning("Não foi possível estimar Duração/Convexidade ou Carry/Roll-down.")

st.markdown("---")
st.subheader("Term Structure de Breakeven de Inflação (snapshot)")
df_be = calcular_curva_breakeven(df_tesouro)
fig_be = grafico_curva_breakeven(df_be)
...
st.markdown("---")
st.subheader("Mapa de Calor de Inclinações (LTN/NTN-F)")
df_tenores, df_spreads = calcular_inclinacoes_prefixados(df_tesouro)
if not df_spreads.empty:
    ult_n = st.slider("Janela (dias)", 30, 250, 90)
    fig_heat = grafico_heatmap_inclinacoes(df_spreads, ult_n=ult_n)
    st.plotly_chart(fig_heat, use_container_width=True)
    # mini KPIs do dia
    hoje = df_spreads.index.max()
    cols = st.columns(df_spreads.shape[1])
    for i, c in enumerate(df_spreads.columns):
        cols[i].metric(c, f"{df_spreads.loc[hoje, c]:+.2f} p.p.")
else:
    st.warning("Spreads por tenores indisponíveis para o período.")
